...
(function(global) {
    ...
    // Log initial state
    console.log('[StorageAdapter] Initial state:', {
    ...
    function _canSync() {
        const checks = {
            ...
            hasUser: !!_state.user,
            hasUserId: !!_state.user?.id,
            hasSupabase: _hasSupabase(),
            isOnline: _state.isOnline,
            navigatorOnline: typeof navigator !== 'undefined' ? navigator.onLine : 'N/A'
        };
        
        // Update isOnline from navigator if needed (belt and suspenders)
        if (typeof navigator !== 'undefined' && navigator.onLine && !_state.isOnline) {
            console.log('[StorageAdapter] Fixing isOnline state from navigator');
            _state.isOnline = true;
            checks.isOnline = true;
        }
        
        const canSync = checks.isAuthenticated && 
                        checks.hasUser && 
                        checks.hasUserId && 
                        checks.hasSupabase && 
                        checks.isOnline;
        ...
        console.log('[StorageAdapter] _canSync() check:', { ...checks, result: canSync });
    ...
    function _syncSOP(sop) {
        console.log('[StorageAdapter] _syncSOP called for:', sop.title);
        
        if (!_canSync()) {
            console.log('[StorageAdapter] _syncSOP SKIPPED - _canSync() returned false');
            return;
        }
        
        console.log('[StorageAdapter] _syncSOP PROCEEDING - calling SupabaseClient.saveSOP()');
        _syncLog('Uploading SOP', { title: sop.title, id: sop.id });
        
        SupabaseClient.saveSOP(sop)
            .then(result => {
                console.log('[StorageAdapter] SupabaseClient.saveSOP result:', result);
                if (result.success) {
                    _syncLog('SOP uploaded OK', { title: sop.title });
                } else {
                    console.error('[StorageAdapter] SOP upload FAILED:', sop.title, result.error);
                }
            })
            .catch(e => {
                console.error('[StorageAdapter] SOP sync EXCEPTION:', sop.title, e);
            });
    }
    ...
    async function _performInitialSync() {
        ...
        try {
            ...
            // Get cloud data
            console.time('[TIMING] sync: cloud fetch (3 parallel)');
            ...
            console.timeEnd('[TIMING] sync: cloud fetch (3 parallel)');
            ...
            if (hasLocal && !hasCloud) {
                // CASE 1: Local only → upload to cloud (background, don't block)
                console.log('[TIMING] sync case: UPLOAD (local→cloud, background)');
                _syncLog('UPLOAD: Local has data, cloud empty - uploading in background');
                
                SupabaseClient.uploadLocalData({
                    sops: localSOPs,
                    folders: localFolders,
                    checklists: localChecklists
                }).then(() => {
                    _set(STORAGE_KEYS.SYNC_STATUS, { lastSync: Date.now(), direction: 'upload' });
                    _syncLog('Background upload complete');
                }).catch(e => {
                    console.warn('[StorageAdapter] Background upload failed:', e);
                });
                
            } else if (!hasLocal && hasCloud) {
                // CASE 2: Cloud only → pull to local
                console.log('[TIMING] sync case: PULL (cloud→local)');
                ...
                console.log('[TIMING] sync case: MERGE (both have data)');
                ...
                // Upload merged data to cloud in background (localStorage is already set above)
                _syncLog('Uploading merged data to cloud (background)');
                
                // Parallel uploads, fire-and-forget — don't block sign-in
                const uploadTasks = [];
                for (const sop of mergedSOPs) {
                    uploadTasks.push(
                        SupabaseClient.saveSOP(sop).catch(e => 
                ...
                for (const folder of mergedFolders) {
                    if (folder.id !== 'general' && !folder.id?.startsWith('folder_default_')) {
                        uploadTasks.push(
                            SupabaseClient.saveFolder(folder).catch(e => 
                ...
                for (const cl of mergedChecklists) {
                    uploadTasks.push(
                        SupabaseClient.saveChecklist(cl).catch(e => 
                            console.warn('[StorageAdapter] Merge upload checklist failed:', e.message)
                        )
                    );
                }
                // Don't await — local data is ready, cloud upload happens in background
                Promise.all(uploadTasks).then(() => {
                    _syncLog('Background merge upload complete');
                }).catch(() => {});
                ...
                console.log('[TIMING] sync case: SKIP (both empty)');
    ...
    const StorageAdapter = {
        ...
        saveSops(sops) {
            ...
            console.log('[StorageAdapter] ========== saveSops() CALLED ==========');
            console.log('[StorageAdapter] SOP count:', sops.length);
            console.log('[StorageAdapter] SOP titles:', sops.map(s => s.title));
            console.log('[StorageAdapter] Auth state:', {
                isAuthenticated: _state.isAuthenticated,
            ...
            const willSync = _canSync();
            console.log('[StorageAdapter] Will sync to cloud:', willSync);
            
            // Sync to cloud if authenticated (fire-and-forget)
            if (willSync) {
                console.log('[StorageAdapter] Starting cloud sync for', sops.length, 'SOPs');
                sops.forEach(sop => _syncSOP(sop));
            } else {
                console.log('[StorageAdapter] Cloud sync SKIPPED - _canSync() returned false');
            }
            
            console.log('[StorageAdapter] ========== saveSops() DONE ==========');
        },
        ...
        Auth: {
            ...
            async signIn(email, password) {
                console.log('[StorageAdapter] signIn() called');
                console.time('[TIMING] signIn total');
                
                if (!_hasSupabase()) {
                    throw new Error('Cloud sync not available');
                }

                console.time('[TIMING] supabase.signInWithPassword');
                const result = await SupabaseClient.signIn(email, password);
                console.timeEnd('[TIMING] supabase.signInWithPassword');
                
                if (result.error) {
                    console.timeEnd('[TIMING] signIn total');
                    throw new Error(result.error);
                }
                ...
                if (result.session && result.user) {
                    ...
                    console.log('[StorageAdapter] Signed in successfully');
                    
                    _notifyAuthListeners(true, result.user);
                    
                    console.time('[TIMING] _performInitialSync');
                    // Perform initial sync after login (blocks until local data is ready)
                    await _performInitialSync();
                    console.timeEnd('[TIMING] _performInitialSync');
                }
                console.timeEnd('[TIMING] signIn total');
