...
(function(global) {
    ...
    const PADDLE_CONFIG = {
        ...
        clientToken: 'live_356c22cf17fc06beab6f072024d',
        
        // Paddle price ID for Pro monthly ($39/mo) — replace with real price ID
        proPriceId: 'pri_01kjgbw1wr5wnxyh2dm63rj76p',
        ...
        customerPortalUrl: 'https://customer-portal.paddle.com/cpl_01kfc0tjfycv2xcvr2vxgvrb3v',
    ...
    const PaddleBilling = {
        ...
        /**
         * Check if subscription has a pending cancellation.
         */
        isCanceling() {
            const detail = this.getPlanDetail();
            return detail?.scheduledCancelAt ? true : false;
        },

        /**
         * Get scheduled cancellation date (or null).
         */
        getCancelDate() {
            const detail = this.getPlanDetail();
            return detail?.scheduledCancelAt || null;
        },

        /**
         * Get formatted cancel date for display (e.g., "March 28, 2026").
         */
        getFormattedCancelDate() {
            const cancelAt = this.getCancelDate();
            ...
            try {
                ...
                if (sub && sub.status === 'active') {
                    this.setPlan('pro', {
                        ...
                        status: sub.status,
                        scheduledCancelAt: sub.scheduledCancelAt || null,
                        source: 'server_sync'
                    });
                    console.log('[PaddleBilling] Pro status restored from server');
                    if (sub.scheduledCancelAt) {
                        console.log(`[PaddleBilling] Cancellation scheduled for ${sub.scheduledCancelAt}`);
                    }
                    
                    window.dispatchEvent(new CustomEvent('withoutme:plan-changed', { 
                        detail: { plan: 'pro', scheduledCancelAt: sub.scheduledCancelAt || null } 
                    }));
                    return 'pro';
                    ...
                    if (sub.status === 'canceled' && sub.currentPeriodEnd) {
                        ...
                        if (periodEnd > new Date()) {
                            // Still within paid period — keep Pro
                            this.setPlan('pro', {
                                subscriptionId: sub.subscriptionId,
                            ...
                            console.log(`[PaddleBilling] Canceled but active until ${sub.currentPeriodEnd} — keeping Pro`);
        ...
        openCheckout(email = null) {
            ...
            try {
                ...
                if (typeof gtag === 'function') gtag('event', 'pro_checkout_opened');
            } catch (e) {
        ...
        _handlePaddleEvent(event) {
            ...
            if (event.name === 'checkout.completed') {
                ...
                // GA4: Track successful Pro upgrade
                if (typeof gtag === 'function') gtag('event', 'pro_upgrade', { value: 39, currency: 'USD' });
        ...
        _showUpgradeSuccess() {
            ...
            toast.textContent = '✅ Welcome to Pro! AI features, team access, and cloud sync are now available.';
        ...
        showPricingModal() {
            ...
            modal.innerHTML = `
                <div class="pricing-modal">
                    ...
                    <p class="pricing-modal-subtitle">Start free. Upgrade when you want AI-powered SOPs and team sharing.</p>
                    ...
                    <div class="pricing-cards">
                        ...
                        <div class="pricing-card pricing-card-pro">
                            ...
                            <ul class="pricing-features">
                                ...
                                <li class="pricing-feature highlight">✅ Describe the task, AI writes the steps</li>
                                ...
                                <li class="pricing-feature">✅ See who completed what and when</li>
                                <li class="pricing-feature">✅ Morning digest email — no check-ins needed</li>
                                <li class="pricing-feature">✅ Assign SOPs to team members with due dates</li>
                                <li class="pricing-feature">✅ Add photos to any step</li>
